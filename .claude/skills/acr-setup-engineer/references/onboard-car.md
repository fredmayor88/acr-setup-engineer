     ...
     **On a refresh of an already-onboarded car, don't ask** — take the template and say so
     ...
     - **User confirms (Yes):** Load every row from the template. Treat each entry as if it
       ...
       **lacks**, still work step 5's ladder: an older template may predate several of these fields,
       ...
       If the template carries `power_torque_chart:` / `gearing_chart:` / `final_drive_chart:` /
       `engine_curve:` (extracted from the game files, so no ladder applies), the charts go on the
       car page in step 7 and the curve is available to every later gearing decision. An
...
9. **Report.** Rows added/updated, and the car's identity facts as written to the `{Car}` page —
   ...
   in, and any template-vs-screenshot discrepancy. Say which of the **power/torque, gearing, and
   final-drive charts** went on the page (and which were skipped and why — no bundled template, or
   a non-adjustable final drive), and if the power/torque curve's peaks disagree with the
   `Max power` / `Max torque` facts, show both figures and explain which is which
   (`notion-structure.md` → *Engine and gearing charts*). Then: any **surface-specific `Gravel` rows**
