...
1. **Resolve the structure by name** (`notion-structure.md` → *Resolution rule*) and create
   ...
   (`notion-rest-read.md` → *Offline mode*); it decides how screenshot cars are handled in step 3.
...
3. **Refresh every car, one after another**, by running `onboard-car.md` → *Refreshing an
   ...
   - **Don't stop for a paste.** Where a per-car refresh would offer or start the paste route for
     a screenshot car's snapshot, keep the car's valid snapshot, note the car, and move on. List
     those cars in the report with the per-car command to run.
   ...
   - **Order the columns once at the end, not per car** for the shared views: each car's own
     ...
     `{Location}` / `{Stage}` views get **one** case-3 call after the last car
...
5. **Report.** One line for the root pages (with the version), then **one line per car** — what
   ...
   - screenshot cars whose snapshot may need the user's rows, with *"refresh the {Car} in my
     Notion"* for each;
