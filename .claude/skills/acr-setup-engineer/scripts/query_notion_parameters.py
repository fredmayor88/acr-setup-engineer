...
Used by the acr-setup-engineer skill to read a filtered slice of a car's Setups rows —
the Notion MCP connector cannot list database rows reliably.

Usage:
  # Setups rows for one car:
  python query_notion_parameters.py <setups_data_source_id> <token> <car_name>

  # Setups the user marked "Learn from this":
  python query_notion_parameters.py <setups_data_source_id> <token> <car_name> --learn-only

  # The car's captured game-default (stock) baseline rows, for the build anchor:
  python query_notion_parameters.py <setups_data_source_id> <token> <car_name> --source default

  # The ordered SHOW property list for a view, computed locally from bundled template
  # YAML(s) — no token/network. A car's catalog is its template file, so this is the only
  # way to build a SHOW list:
...
Options:
  ...
  --show-order   Print the ordered SHOW property list for a Setups view (see below)
                 instead of rows: Name, then value columns by their parameters'
